...
public final class BlockSignatureBuffer {
    ...
    public static byte[] getBufferSignature(PublicKey nextPubKey, byte[] data, Optional<ExternalSignature> externalSignature) {
        var buffer = ByteBuffer.allocate(HEADER_SIZE + data.length + nextPubKey.toBytes().length + externalSignature.map((a) -> a.getSignature().length).orElse(0)).order(ByteOrder.LITTLE_ENDIAN);
        buffer.put(data);
    ...
    public static byte[] getBufferSealedSignature(PublicKey nextPubKey, byte[] data, byte[] blockSignature) {
        var buffer = ByteBuffer.allocate(HEADER_SIZE + data.length + nextPubKey.toBytes().length + blockSignature.length).order(ByteOrder.LITTLE_ENDIAN);
        buffer.put(data);
        buffer.putInt(nextPubKey.getAlgorithm().getNumber());
        buffer.put(nextPubKey.toBytes());
